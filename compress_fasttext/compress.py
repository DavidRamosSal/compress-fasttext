...
class CompressedFastTextKeyedVectors(FastTextKeyedVectors):
    ...
    def update_index2word(self):
        if not self.index_to_key:
            ...
            self.index_to_key = [
                inverse_index.get(i) for i in range(len(self.key_to_index))
            ]
    ...
    def word_vec(self, word, use_norm=False):
        ...
        if word in self.key_to_index:
            ...
            ngram_hashes = ft_ngram_hashes(
                word=word, minn=self.min_n, maxn=self.max_n, num_buckets=self.bucket
            )
    ...
    def fill_norms(self, force=False):
        ...
        if self.norms is None or force:
            ...
            self.norms = np.stack(
                [sum(self.vectors[i] ** 2) ** 0.5 for i in range(len(self.vectors))]
            )
...
def make_new_fasttext_model(
    ...
    new_ft.vectors_vocab = (
        None  # if we don't fine tune the model we don't need these vectors
    )
...
def quantize_ft(ft, qdim=100, centroids=255, sample=None):
    ...
    new_vectors = quantize(
        ft.vectors, qdim=qdim, centroids=centroids, verbose=True, sample=sample
    )
    logger.info("quantizing ngrams...")
    new_vectors_ngrams = quantize(
        ft.vectors_ngrams, qdim=qdim, centroids=centroids, verbose=True, sample=sample
    )

    return make_new_fasttext_model(
        ft, new_vectors=new_vectors, new_vectors_ngrams=new_vectors_ngrams
    )


def svd_ft(ft, n_components=30, fp16=True):
    logger.info("compressing vectors...")
    new_vectors = DecomposedMatrix.compress(
        ft.vectors, n_components=n_components, fp16=fp16
    )
    logger.info("compressing ngrams...")
    new_vectors_ngrams = DecomposedMatrix.compress(
        ft.vectors_ngrams, n_components=n_components, fp16=fp16
    )
...
def prune_ft_freq(
    ...
    new_to_old_buckets, old_hash_count = count_buckets(
        ft,
        list(ft.key_to_index.keys()),
        new_ngrams_size=new_ngrams_size,
    )
    logger.info("old ngrams in use: {}".format(len(old_hash_count)))
    id_and_count = sorted(
        old_hash_count.items(), key=lambda x: scorer(*x), reverse=True
    )
    ...
    rsm = RowSparseMatrix.from_small(
        ids, top_ngram_vecs, nrows=ft.vectors_ngrams.shape[0]
    )
